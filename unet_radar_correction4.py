...
def train(cfg: Config):
    ...
    if is_main:
        ...
        dl_va = DataLoader(
            ...
            batch_size=cfg.batch_size,
            shuffle=False,
            num_workers=cfg.val_num_workers,  # FIX: use validation workers
            pin_memory=True,
            persistent_workers=cfg.persistent_workers,
            prefetch_factor=cfg.prefetch_factor,
            collate_fn=safe_collate
    ...
    # FIX: Determine DEM availability correctly using dataset attributes
    has_dem = bool(
        cfg.use_dem and (
    ...
    model = model.to(memory_format=torch.channels_last)  # FIX: channels_last for model
    ...
    start_epoch = 1
    # --- Optimizer & Scheduler created above this point ---
...
def main():
    ...
    # FIX: assign even if 0 were allowed; we keep behavior with explicit None
